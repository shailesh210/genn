...
\brief File generated from GeNN for the model MBody_individualID containing general control code.
*/
//-------------------------------------------------------------------------
...
cudaHostAlloc(&gpPNKC, 3126 * sizeof(uint32_t), cudaHostAllocPortable);
    deviceMemAllocate(&d_gpPNKC, dd_gpPNKC, 3126 * sizeof(uint32_t));
...
cudaFreeHost(gpPNKC);
    CHECK_CUDA_ERRORS(cudaFree(d_gpPNKC));
