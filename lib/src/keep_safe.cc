...
//	cl_kernel kernel2 = NULL;
	...
	//	cl_uint deviceCount;				//replace ret_num_devices to deviceCount
	//	cl_uint ret_num_platforms;
	//	cl_int ret;
	//	CHECK_OPENCL_ERRORS(clGetPlatformIDs(1, &platform_id, &ret_num_platforms));
	

	//	CHECK_OPENCL_ERRORS(clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_GPU, 1, &device_id, &deviceCount));
	    ...
	    cudaFuncAttributes krnlAttr[2][krnlNo];									//whats this?
	    CUfunction kern;
	    CUresult res;
		...
		#ifdef OPENCL
			for (int blkSz = 1, mx= deviceProp[theDevice].MAX_WORK_GROUP_SIZE / warpSize; blkSz <= mx; blkSz++) {
		#else
		...
		#ifdef OPENCL
			blockLimit = floor((float) deviceProp[theDevice].MAX_WORK_UNITS_PER_COMPUTE_UNIT/warpSize/blkSz);
		#else
		...
		#ifdef OPENCL
			blockLimit = floor(deviceProp[theDevice].REGISTERSS_PER_BLOCK/blockLimit);
		#else
		...
		#ifdef OPENCL
			blockLimit = floor(deviceProp[theDevice].REGISTERSS_PER_BLOCK/blockLimit/warpAllocGran)*warpAllocGran;
		#else
		...
		#ifdef OPENCL
			blockLimit = floor(deviceProp[theDevice].DEVICE_LOCAL_MEM_SIZE/blockLimit);
		#else
		...
		#ifdef OPENCL
			if (requiredBlocks <= (mainBlockLimit*deviceProp[theDevice].DEVICE_MAX_COMPUTE_UNITS)) {
		#else
		...
		#ifdef OPENCL
			deviceOccupancy[kernel][theDevice]= blkSz*mainBlockLimit*deviceProp[theDevice].DEVICE_MAX_COMPUTE_UNITS;
		#else
		...
		#ifdef OPENCL
			int newOccupancy= blkSz*mainBlockLimit*deviceProp[theDevice].DEVICE_MAX_COMPUTE_UNITS;
		#else
