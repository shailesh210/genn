...
//	os << "extern CStopWatch neuron_timer;" << ENDL;
  ...
  and making g member a synapse variable.*/" << ENDL;
...
#ifndef OPENCL
	os << "// ------------------------------------------------------------------------" << ENDL;
    os << "// Function for mapping OPENCL buffers." << ENDL;
    os << ENDL;
    os << "void mapBuffer();" << ENDL;
    os << ENDL;
#endif
...
#ifndef OPENCL
	os << "// ------------------------------------------------------------------------" << ENDL;
    os << "// Function for unmapping OPENCL buffers." << ENDL;
    os << ENDL;
    os << "void unmapBuffer();" << ENDL;
    os << ENDL;
#endif
	...
	#ifndef OPENCL
	//-----------------------------------------------
	// For OpenCL devices. Function to Map buffer objects to varaibles 
	// Use of pinned memory for optimization
...
/////////////////
	...
	for (int j = 0; j < nModels[nt].varNames.size(); j++) {
		...
		os << nModels[nt].varNames[j] << model.neuronName[i] << "= (" << nModels[nt].varTypes[j] << " *) clEnqueueMapBuffer(command_queue, d_" << nModels[nt].varNames[j] << model.neuronName[i];   //check d_ or dd_
		os << ", CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, " << size << "* sizeof(" << nModels[nt].varTypes[j] << ")";
		os << ", 0, NULL, NULL, &ret);" << ENDL;
	...
	//-----------------------------------------------
	// For OpenCL devices. Function to UnMap buffer objects to varaibles 
	// Use of pinned memory for optimization
...
/////////////////
	...
	for (int j = 0; j < nModels[nt].varNames.size(); j++) {
		...
		os << "clEnqueueUnmapMemObject(command_queue, d_"<< nModels[nt].varNames[j] << model.neuronName[i] << ", " << nModels[nt].varNames[j] << model.neuronName[i] <<", 0, NULL, NULL);" << ENDL; 
    ...
    os << "{" << ENDL;
       ...
       os << "FILE *fp;" << ENDL;
       os << "char fileName1[] = \"./SynDelay_CODE/neuronKrnl.cl\";" << ENDL;
       os << "char fileName2[] = \"./SynDelay_CODE/synapseKrnl.cl\";" << ENDL;
       ...
       os << "char *buffer = (char*)calloc(len, sizeof(char));              // to debug print log" << ENDL;
       os << "ret = clGetProgramBuildInfo(program, device_ids[" << theDevice << "], CL_PROGRAM_BUILD_LOG, len, buffer, NULL);" << ENDL;
       os << "cout << buffer;" << ENDL;
       os << "CHECK_OPENCL_ERRORS(ret);" << ENDL;
	...
	os << "d_t "  << "=clCreateBuffer(context,  CL_MEM_READ_WRITE ,";		//check d_ or dd_
		os << size << " * sizeof(float), NULL, &ret);" << ENDL;
		os << "CHECK_OPENCL_ERRORS(ret);"<<ENDL;
		
		
	os << "CHECK_OPENCL_ERRORS(clEnqueueReadBuffer(command_queue, d_t" ;
	...
	if (model.neuronDelaySlots[i] > 1) {
       ...
       os << "    spkQuePtr" << model.neuronName[i] << "[0] = 0;" << ENDL;
        ...
        os << "//unmap spkQuePtr" << model.neuronName[i] << ENDL;
        os << "clEnqueueUnmapMemObject(command_queue, d_spkQuePtr"<< model.neuronName[i] << ",spkQuePtr" << model.neuronName[i] <<", 0, NULL, NULL);" << ENDL; 
...
//	os << "#include \"synapseKrnl.cc\"" << ENDL;
 //   }
	...
	if ((model.neuronNeedTrueSpk[i]) && (model.neuronDelaySlots[i] > 1)) {
		...
		os << ", CL_TRUE, 0, spkQuePtr" << model.neuronName[i] <<"*sizeof(unsigned int)";
		os << ", glbSpkCnt" << model.neuronName[i] << ",0, NULL, NULL ));" << ENDL;
		  
		os << "CHECK_OPENCL_ERRORS(clEnqueueWriteBuffer(command_queue, d_glbSpk" << model.neuronName[i];
		
		os << ", CL_TRUE, 0, " << "(glbSpkCnt" << model.neuronName[i] << "[spkQuePtr" << model.neuronName[i] << "]+(spkQuePtr" << model.neuronName[i] << "*" << model.neuronN[i] << ")) * sizeof(unsigned int)";
		os << ", glbSpk" << model.neuronName[i] << ",0, NULL, NULL ));" << ENDL;
...
//////////////////////
	...
	if (model.neuronNeedSpkEvnt[i]) {
	  if (model.neuronDelaySlots[i] > 1) {
		...
		os << "CHECK_OPENCL_ERRORS(clEnqueueReadBuffer(command_queue, d_glbSpkEvnt" << model.neuronName[i] ;
		os << ",CL_TRUE, 0, (glbSpkCntEvnt" << model.neuronName[i] << "[spkQuePtr" << model.neuronName[i] << "]+(spkQuePtr" <<  model.neuronName[i] << "*" << model.neuronN[i] << "))  * sizeof(unsigned int)";
    ...
    for (int i= 0, l= model.synapseDynamicsKernelParameters.size(); i < l; i++) {
        os << "CHECK_OPENCL_ERRORS(clSetKernelArg(calcSynapseDynamics, " << param_count++ << ", sizeof(cl_mem), &" << model.synapseDynamicsKernelParameters[i] <<"));" << ENDL;
    ...
    if (model.synapseConnType[i] == SPARSE) {
        os << "CHECK_OPENCL_ERRORS(clSetKernelArg(calcSynapseDynamics, " << param_count++ << ", sizeof(cl_mem), &d_C" << model.synapseName[i] <<"));" << ENDL;
//      os << "extern SparseProjection C" << model.synapseName[i] << ";" << ENDL;
    }
    if (model.synapseGType[i] == INDIVIDUALG) { // not needed for GLOBALG, INDIVIDUALID
        for (int k = 0, l = weightUpdateModels[st].varNames.size(); k < l; k++) {
    ...
    if (model.synapseConnType[i] == SPARSE) {
        os << "CHECK_OPENCL_ERRORS(clSetKernelArg(calcSynapses, " << param_count++ << ", sizeof(cl_mem), &d_C" << model.synapseName[i] <<"));" << ENDL;
//      os << "extern SparseProjection C" << model.synapseName[i] << ";" << ENDL;
    }
    if (model.synapseGType[i] == INDIVIDUALG) { // not needed for GLOBALG, INDIVIDUALID
        for (int k = 0, l = weightUpdateModels[st].varNames.size(); k < l; k++) {
    ...
    for (int k = 0, l= weightUpdateModels[st].extraGlobalSynapseKernelParameters.size(); k < l; k++) {
        os << "CHECK_OPENCL_ERRORS(clSetKernelArg(calcSynapses, " << param_count++ << ", sizeof(cl_mem), &d_" << weightUpdateModels[st].extraGlobalSynapseKernelParameters[k] << model.synapseName[i] <<"));" << ENDL;
    ...
    if (model.synapseConnType[i] == SPARSE) {
        os << "CHECK_OPENCL_ERRORS(clSetKernelArg(learnSynapsesPost, " << param_count++ << ", sizeof(cl_mem), &d_C" << model.synapseName[i] <<"));" << ENDL;
//      os << "extern SparseProjection C" << model.synapseName[i] << ";" << ENDL;
    }
    if (model.synapseGType[i] == INDIVIDUALG) { // not needed for GLOBALG, INDIVIDUALID
        for (int k = 0, l = weightUpdateModels[st].varNames.size(); k < l; k++) {
    ...
    for (int k = 0, l= nModels[nt].extraGlobalNeuronKernelParameters.size(); k < l; k++) {
        ...
        os << "CHECK_OPENCL_ERRORS(clSetKernelArg(calcNeurons, " << param_count++ << ", sizeof(cl_mem), &d_" << nModels[nt].extraGlobalNeuronKernelParameters[k] << model.neuronName[i] <<"));" << ENDL;
    //    extern_variable_def(os, nModels[nt].extraGlobalNeuronKernelParameterTypes[k], nModels[nt].extraGlobalNeuronKernelParameters[k]+model.neuronName[i]);
    }   
    }
    ...
    if (model.synapseConnType[i] == SPARSE) {
        os << "CHECK_OPENCL_ERRORS(clSetKernelArg(calcNeurons, " << param_count++ << ", sizeof(cl_mem), &d_C" << model.synapseName[i] <<"));" << ENDL;
//      os << "extern SparseProjection C" << model.synapseName[i] << ";" << ENDL;
    }
    if (model.synapseGType[i] == INDIVIDUALG) { // not needed for GLOBALG, INDIVIDUALID
        for (int k = 0, l = weightUpdateModels[st].varNames.size(); k < l; k++) {
...
//	os << "size_t nGlovbalSize = " << sqGridSize * neuronBlkSz << ","<< sqGridSize <<");" << ENDL;
 //   }
	...
	if (model.synDynGroups > 0) {
	    ...
	    os << "CHECK_OPENCL_ERRORS(clEnqueueNDRangeKernel(command_queue,calcSynapseDynamics,1, NULL, &sDGlobalSize , &sDLocalSize, 0, NULL, NULL/*&synDynevent*/));"<< ENDL;
	...
	if (model.timing) os << "cudaEventRecord(synapseStart);" << ENDL; 
		    os << "CHECK_OPENCL_ERRORS(clEnqueueNDRangeKernel(command_queue,calcSynapses,1, NULL, &sGlobalSize , &sLocalSize, 0, NULL,NULL/* &synapseevent*/));"<< ENDL;
	        os << "CHECK_OPENCL_ERRORS(clFinish(command_queue));" << ENDL;
/*	for (int i= 0, l= model.synapseKernelParameters.size(); i < l; i++) {
	...
	if (model.lrnGroups > 0) {
	    if (model.timing) os << "cudaEventRecord(learningStart);" << ENDL;
		 os << "CHECK_OPENCL_ERRORS(clEnqueueNDRangeKernel(command_queue,learnSynapsesPost,1, NULL, &lGlobalSize , &lLocalSize, 0, NULL,NULL/* &learningevent*/));"<< ENDL;
	       os << "CHECK_OPENCL_ERRORS(clFinish(command_queue));" << ENDL;
         /*   for (int i= 0, l= model.simLearnPostKernelParameters.size(); i < l; i++) {
    ...
    if (model.timing) os << "cudaEventRecord(neuronStart);" << ENDL;
		os << "CHECK_OPENCL_ERRORS(clEnqueueNDRangeKernel(command_queue,calcNeurons,1, NULL, &nGlobalSize , &nLocalSize, 0, NULL,NULL/* &learningevent*/));"<< ENDL;
        os << "CHECK_OPENCL_ERRORS(clFinish(command_queue));" << ENDL;
